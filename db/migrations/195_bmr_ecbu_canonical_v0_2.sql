...
-- Migration 195: BMR / ECBU canonical LAB/LOINC consolidation
-- Version: 0.2.0
-- Date: 2026-09-25
-- Requires: 194_laboratory_terminology_foundation.sql
-- Target: PostgreSQL / Neon-compatible
--
-- This migration extends the canonical laboratory model. It does NOT create a
-- BMR-specific terminology hierarchy and does NOT duplicate the LOINC layer.
--
...
--
-- Additions:
--   lab_report -> lab_specimen (physical specimen instance)
--   lab_report -> microbiology_isolate
--   microbiology_isolate -> lab_observation (reported organism identification)
...
--   laboratory facts use lab_observation.loinc_code / loinc_concept_id created
--   by migration 194. BMR/ECBU never creates its own terminology tables.
--
-- Important:
...
SELECT pg_advisory_xact_lock(hashtext('IML:BMR_ECBU:v0.2.0:CANONICAL'));
...
BEGIN
  FOREACH obj IN ARRAY expected_objects LOOP
    IF to_regclass(obj) IS NULL THEN
      RAISE EXCEPTION 'BMR_ECBU_v0.2.0 aborted: required object % is missing', obj;
    END IF;
    ...
    IF id_type IS NULL THEN
      RAISE EXCEPTION 'BMR_ECBU_v0.2.0 aborted: %.id is missing', obj;
    END IF;

    IF id_type <> 'uuid' THEN
      RAISE EXCEPTION 'BMR_ECBU_v0.2.0 aborted: %.id must be uuid, found %',
        obj, id_type;
    END IF;
  END LOOP;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 aborted: expected lab_observation.lab_report_id uuid NOT NULL';
  END IF;

  -- Migration 194 is the canonical LOINC/UCUM foundation.
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'iml_laboratory'
    ...
    WHERE table_schema = 'iml_laboratory'
      AND table_name = 'lab_observation'
      AND column_name = 'normalized_unit_system'
  ) THEN
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 aborted: migration 194 LAB/LOINC foundation is missing';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM iml_terminology.system
    WHERE system_code = 'LOINC'
  ) OR NOT EXISTS (
    SELECT 1
    FROM iml_terminology.release
    WHERE release_key = 'LOINC_2.83'
  ) THEN
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 aborted: canonical LOINC 2.83 metadata is missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 aborted: expected microbiology_context.lab_report_id uuid NOT NULL';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 aborted: expected antimicrobial_susceptibility.lab_report_id uuid NOT NULL';
  END IF;
END
$$;
...
'Physical specimen normalized separately from order/report. In BMR/ECBU v0.2.0 the main specimen is urine. A missing local order does not prevent representation of an externally received laboratory report.';
...
-- Existing v0.1.1 test databases already have microbiology_isolate. Add the
-- canonical observation link explicitly so this migration upgrades them too.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'fk_microbiology_isolate_identification_observation'
      AND conrelid = 'iml_laboratory.microbiology_isolate'::regclass
  ) THEN
    ALTER TABLE iml_laboratory.microbiology_isolate
      ADD CONSTRAINT fk_microbiology_isolate_identification_observation
      FOREIGN KEY (identification_observation_id)
      REFERENCES iml_laboratory.lab_observation(id)
      ON DELETE RESTRICT;
  END IF;
END
...
COMMENT ON COLUMN iml_laboratory.microbiology_isolate.identification_observation_id IS
'Optional link to the canonical LOINC-coded lab_observation that reported the organism identification.';
...
CREATE INDEX IF NOT EXISTS idx_microbiology_isolate_identification_observation
  ON iml_laboratory.microbiology_isolate(identification_observation_id)
  WHERE identification_observation_id IS NOT NULL;
...
-- Guard against linking an isolate to an identification observation from
-- another report. The observation remains the canonical LOINC-coded fact.
...
BEGIN
  IF NEW.identification_observation_id IS NULL THEN
    RETURN NEW;
  END IF;

  SELECT lab_report_id
    INTO observation_report_id
    FROM iml_laboratory.lab_observation
   WHERE id = NEW.identification_observation_id;

  IF observation_report_id IS NULL THEN
    RAISE EXCEPTION
      'BMR_ECBU: identification observation % does not exist',
      NEW.identification_observation_id;
  END IF;

  IF observation_report_id <> NEW.lab_report_id THEN
    RAISE EXCEPTION
      'BMR_ECBU: isolate report % does not match identification observation report %',
      NEW.lab_report_id, observation_report_id;
  END IF;
...
CREATE TRIGGER trg_bmr_ecbu_isolate_observation
BEFORE INSERT OR UPDATE OF lab_report_id, identification_observation_id
ON iml_laboratory.microbiology_isolate
FOR EACH ROW
EXECUTE FUNCTION iml_laboratory.bmr_ecbu_check_isolate_observation();
...
BEGIN
  IF to_regclass('iml_laboratory.lab_specimen') IS NULL THEN
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: lab_specimen missing';
  END IF;

  IF to_regclass('iml_laboratory.microbiology_isolate') IS NULL THEN
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: microbiology_isolate missing';
  END IF;

  IF to_regclass('iml_clinical.microbiology_interpretation') IS NULL THEN
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: microbiology_interpretation missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: lab_report.specimen_id missing or not uuid';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: microbiology_isolate.identification_observation_id missing or not uuid';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 failed post-check: antimicrobial_susceptibility.isolate_id missing or not uuid';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'BMR_ECBU_v0.2.0 post-check: lab_observation.specimen_id unexpectedly exists; canonical route must remain lab_observation -> lab_report -> lab_specimen';
  END IF;
END
$$;
