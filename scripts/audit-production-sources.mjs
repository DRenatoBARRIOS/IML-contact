...
async function probeOnce(url) {
  ...
  try {
    ...
    try {
      ...
      // The status and final URL are sufficient for this transport probe.
    }
...
function consolidateAttempts(attempts) {
  ...
  if (attempts.length >= 2 && attempts.every((item) => item.classification === "broken")) {
    return "confirmed_broken";
  }
  if (attempts.every((item) => item.classification === "blocked")) return "blocked";
  if (attempts.every((item) => ["transient", "fetch_error"].includes(item.classification))) {
    return "transient";
  }
  return "inconclusive";
}
...
async function probeWithRetry(url) {
  ...
  if (first.classification === "reachable") {
    return { outcome: "reachable", attempts: [first] };
  }

  await sleep(RETRY_DELAY_MS);
  const second = await probeOnce(url);
  return {
    outcome: consolidateAttempts([first, second]),
...
function inspectMetadata(country, source, duplicatePublicUrls, duplicateDocumentaryUrls) {
  const errors = [];
  const warnings = [];
  ...
  if (source.url && !PUBLISHABLE.has(source.url_status)) {
    errors.push("public_url_exposed_with_non_publishable_status");
  }

  if (!source.url && source.url_status === "verified" && !source.replacement_reason) {
    warnings.push("verified_documentary_only_without_explanation");
  }

  const checkedAge = ageDays(source.last_checked_at);
  if (checkedAge == null) {
    warnings.push("missing_last_validated_at");
  } else if (checkedAge > MAX_VALIDATED_AGE_DAYS) {
    warnings.push("validated_review_due");
  }

  if (!source.note) warnings.push("missing_evidence_note");

  if (!Array.isArray(source.indicators) || source.indicators.length === 0) {
    errors.push("source_without_indicator");
  } else {
  ...
  if (source.documentary_url && duplicateDocumentaryUrls.has(source.documentary_url)) {
    warnings.push("duplicate_documentary_url_within_country");
  }
...
async function main() {
  ...
  if (!response.ok) {
    throw new Error(`Country API returned HTTP ${response.status}`);
  }
  ...
  for (const country of countries) {
    ...
    for (const source of sources) {
      const metadata = inspectMetadata(country, source, duplicatePublicUrls, duplicateDocumentaryUrls);
      sourceRows.push({
        iso3: country.iso3,
  ...
  const results = sourceRows.map((row) => {
    ...
    if (probe?.outcome === "confirmed_broken") {
      errors.push("public_link_confirmed_broken_404_410");
    } else if (probe?.outcome === "blocked") {
      warnings.push("automated_probe_blocked_secondary_check_required");
    } else if (probe?.outcome === "transient") {
      warnings.push("automated_probe_transient_secondary_check_required");
    } else if (probe?.outcome === "inconclusive") {
      warnings.push("automated_probe_inconclusive_secondary_check_required");
    }

    if (probe?.outcome === "reachable") {
      const lastAttempt = [...probe.attempts].reverse().find((item) => item.classification === "reachable");
      if (row.stored_status === "verified" && lastAttempt?.redirected) {
        warnings.push("stored_verified_but_redirects_now");
      }
      if (row.stored_status === "redirected" && !lastAttempt?.redirected) {
        warnings.push("stored_redirected_but_direct_now");
      }
    }

    return {
      ...row,
      probe: probe || { outcome: "not_public", attempts: [] },
