...
function classifyHttpStatus(status) {
  ...
  if (status === 408 || status === 425 || status === 429 || status >= 500) return "transient_error";
  return "unverified";
}
...
function withGate(source, result) {
  ...
  return { ...result, gate, secondary_check_required: gate === "warning" };
}
...
export async function auditManifest(manifestPath) {
  ...
  const results = [];
  for (const source of manifest.sources || []) results.push(await auditSource(source));
