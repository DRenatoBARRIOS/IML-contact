...
const styles = `
  ...
  * { box-sizing: border-box; }
  ...
  .status-message { font-size: 14px; line-height: 1.6; }
...
const CONTACT_FORM_ACTION = "https://formspree.io/f/yourFormId";
...
function WorldPage() {
  ...
  return <section className="section"><div className="container"><SectionTitle badge="Country notes" title="A growing global review rather than a closed country list" text="The countries currently included in the study are not presented as a fixed or exhaustive panel. They are analysed progressively, in a partly opportunistic sequence, with the longer-term objective of extending the review to as many countries as possible." /><Card className="soft-card"><div className="content-block"><h3>Reading guide for score ranges</h3><div className="tile-grid">{[{range:"0-39%",title:"Foundational stage",text:"Very limited interoperability maturity. Core structures, exchange routines and trust mechanisms remain fragmented or embryonic."},{range:"40-59%",title:"Emerging stage",text:"A visible trajectory exists, but operational continuity remains uneven and several core layers are still incomplete."},{range:"60-79%",title:"Structured stage",text:"The system shows a coherent interoperability architecture with meaningful deployment, while important gaps still affect scale or consistency."},{range:"80-100%",title:"Advanced stage",text:"Interoperability is broadly structured, operational and legible, with stronger continuity across identity, governance, exchange and real use."}].map((item) => <div key={item.range} className="mini-tile"><div className="mini-tile-title">{item.range} · {item.title}</div><div className="mini-tile-text">{item.text}</div></div>)}</div></div></Card><div className="select-wrap top-gap"><label>Choose a country</label><select value={selectedCountry} onChange={(event) => setSelectedCountry(event.target.value)}><option value="">Choose a country</option>{options.map((country) => <option key={country} value={country}>{country}</option>)}</select></div><div className="top-gap"><WorldMap selectedCountry={selectedCountry} onSelect={setSelectedCountry} /></div>{selectedCountry ? <div className="top-gap"><CountryProfile selectedCountry={selectedCountry} /></div> : null}</div></section>;
}
function ContactPage() {
  ...
  return <section className="section"><div className="container"><SectionTitle badge="" title="Editorial and institutional contact" text="For publication matters, country notes, methodological discussion and institutional exchanges, please use the form below or the direct contact details." /><div className="split-grid profile-grid"><Card><div className="content-block"><h3>Contact form</h3><form className="contact-form" action={CONTACT_FORM_ACTION} method="POST" target="_top"><input type="hidden" name="_subject" value="IML website contact request" /><input type="hidden" name="_captcha" value="false" /><div className="form-grid"><label>Name<input name="name" placeholder="Your name" required /></label><label>Email<input name="email" placeholder="name@organisation.org" type="email" required /></label></div><label>Organisation<input name="organisation" placeholder="Institution, ministry, donor, provider, insurer..." /></label><label>Message<textarea name="message" placeholder="Describe your request or the update you want to discuss." rows="6" required /></label><div className="form-actions"><button type="submit" className="primary-button">Send</button><span className="form-note">Simple HTML form. Replace the endpoint once in the code and it posts directly.</span></div></form></div></Card><div className="stack-layout"><Card className="soft-card"><div className="content-block"><h3>Direct contact</h3><div className="mail-box">iml.health@pm.me</div><div className="form-actions top-gap-small"><button type="button" className="secondary-button" onClick={copyEmail}>Copy email</button>{copied ? <span className="form-note">Email copied.</span> : null}</div></div></Card><Card><div className="content-block"><h3>Typical use cases</h3><ul className="plain-list"><li>• Request a country note or benchmark update</li><li>• Discuss the methodology with institutional partners</li><li>• Open an editorial or publication conversation</li><li>• Explore an identity-layer or medicines-analysis use case</li><li>• Extend the U.S. sample-state section or add new countries</li></ul></div></Card></div></div></div></section>;
}
function Footer() {
