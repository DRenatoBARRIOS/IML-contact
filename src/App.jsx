...
// Canonical ISO handling. The GeoJSON uses DEU for Germany, while some
// databases and older datasets use GER or DE. All variants must resolve to DEU.
const ISO3_ALIASES = Object.freeze({
  DE: "DEU",
  DEU: "DEU",
  GER: "DEU",
});

function canonicalIso3(value, name = "") {
  const raw = String(value || "").trim().toUpperCase();
  if (ISO3_ALIASES[raw]) return ISO3_ALIASES[raw];

  const normalizedName = String(name || "").trim().toLowerCase();
  if (normalizedName === "germany" || normalizedName === "deutschland" || normalizedName === "allemagne") {
    return "DEU";
  }
...
function WorldMap({ profiles, selectedCountry, onSelect }) {
  ...
  const profileByIso3 = useMemo(
    () =>
      Object.fromEntries(
        profiles
          .map((profile) => [canonicalIso3(profile?.iso3, profile?.name), profile])
        ...
        <svg viewBox={`0 0 ${MAP_WIDTH} ${MAP_VISIBLE_HEIGHT}`} className="world-map" aria-label="Interactive IML world map">
          ...
          <g>
            {mapFeatures.map((feature) => {
              const iso3 = featureIso3(feature);
              const profile = profileByIso3[iso3];
              const selected = canonicalIso3(selectedCountry?.iso3, selectedCountry?.name) === iso3;
...
function WorldPage() {
  ...
  const selectedProfile = selectedCountry
    ? profileByIso3[canonicalIso3(selectedCountry.iso3, selectedCountry.name)]
    : null;
