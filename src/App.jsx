...
function normalizeProfile(profile) {
  ...
  return {
    ...
    overallScore: Number(
      ...
      profile.assessment?.overall_score ??
      profile.assessment?.overallScore ??
      0
    ),
...
function averageScore(values = [], overallScore = null) {
  const storedScore = Number(overallScore);
  if (Number.isFinite(storedScore) && storedScore > 0) return Math.round(storedScore);
...
function metricScore(profile, metric) {
  ...
  if (metric === "overall") return averageScore(profile.values, profile.overallScore);
...
function countryReportText(profile) {
  const lines = [
    ...
    `ISO3: ${profile.iso3}`,
    `Overall exploratory signal: ${averageScore(profile.values, profile.overallScore)}/100`,
...
function CountryReport({ profile }) {
  ...
  const overall = averageScore(profile.values, profile.overallScore);
            ...
            <div className="profile-head">
              <div>
                ...
                <div className="profile-title-row">
                  ...
                  >
                    {averageScore(profile.values, profile.overallScore)}/100
                  </div>
                </div>
              </div>
            </div>
