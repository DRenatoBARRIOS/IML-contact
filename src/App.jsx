...
function ContactPage() {
  ...
  return <section className="section"><div className="container"><SectionTitle badge="" title="Editorial and institutional contact" text="For publication matters, country notes, methodological discussion and institutional exchanges, please use the form below or the direct contact details." /><div className="split-grid profile-grid"><Card><div className="content-block"><h3>Contact form</h3><form className="contact-form" onSubmit={handleSubmit}><div className="form-grid"><label>Name<input value={form.name} onChange={(event) => handleChange("name", event.target.value)} placeholder="Your name" required /></label><label>Email<input value={form.email} onChange={(event) => handleChange("email", event.target.value)} placeholder="name@organisation.org" type="email" required /></label></div><label>Organisation<input value={form.organisation} onChange={(event) => handleChange("organisation", event.target.value)} placeholder="Institution, ministry, donor, provider, insurer..." /></label><label>Message<textarea value={form.message} onChange={(event) => handleChange("message", event.target.value)} placeholder="Describe your request or the update you want to discuss." rows="6" required /></label><div className="form-actions"><button type="submit" className="primary-button">Send</button>{sent ? <span className="status-message status-success">Preview OK</span> : null}</div></form></div></Card><div className="stack-layout"><Card className="soft-card"><div className="content-block"><h3>Direct contact</h3><div className="mail-box">iml.health@pm.me</div><div className="form-actions top-gap-small"><button type="button" className="secondary-button" onClick={copyEmail}>Copy email</button>{copied ? <span className="form-note">Email copied.</span> : null}</div></div></Card><Card><div className="content-block"><h3>Typical use cases</h3><ul className="plain-list"><li>• Request a country note or benchmark update</li><li>• Discuss the methodology with institutional partners</li><li>• Open an editorial or publication conversation</li><li>• Explore an identity-layer or medicines-analysis use case</li><li>• Extend the U.S. sample-state section or add new countries</li></ul></div></Card></div></div></div></section>;
}
function Footer() {
