...
function featureIso3(feature) {
  ...
  return String(properties.iso3 || properties.ISO_A3 || properties.adm0_a3 || properties.ADM0_A3 || "")
    .trim()
    .toUpperCase();
}
...
function WorldMap({ profiles, selectedCountry, onSelect }) {
  ...
  const profileByIso3 = useMemo(() => Object.fromEntries(profiles.map((profile) => [profile.iso3, profile])), [profiles]);
    ...
    <div className="world-box">
      ...
      <div className="world-map-wrap map-stage">
        <svg viewBox={`0 0 ${MAP_WIDTH} ${MAP_VISIBLE_HEIGHT}`} className="world-map" aria-label="Interactive IML world map">
          ...
          <g>
            {worldCountries.features.filter((feature) => !isAntarcticaFeature(feature)).map((feature) => {
              const iso3 = featureIso3(feature);
              const profile = profileByIso3[iso3];
        ...
        <Card>
          <div className="content-block">
            ...
            <div className="profile-stat-grid">
              {AXES.map((axis, index) => <div className="profile-stat" key={axis}><span>{axis}</span><strong>{profile.values?.[index] ?? 0}</strong></div>)}
            </div>
          </div>
        </Card>
    ...
    <>
      <section className="hero"><div className="container hero-grid">
        <div className="hero-copy">
          ...
          <p className="hero-text">The current website is a public review interface and PostgreSQL test environment. Country profiles remain documented working analyses pending institutional and local expert review.</p>
          <Card className="note-box"><p>IML does not rank countries. It creates multidimensional profiles, documents evidence and uncertainty, and identifies practical improvement pathways.</p></Card>
          <div className="button-row">
            <a className="primary-button" href={MANUSCRIPT_URL} download="IML_Founding_Manuscript.pdf">Download the Founding Manuscript</a>
...
function MethodologyPage() {
  const domains = [
    ["Governance and Standards", "GOV", "Responsibilities, standards, legal clarity and accountable ecosystem governance."],
    ["Technical Interoperability", "TEC", "Secure, reliable and maintainable exchange across heterogeneous systems."],
    ["Identity, Consent and Trust", "ID", "Reliable identification, appropriate consent, provenance and confidence."],
    ["Adoption and Use", "USE", "Integration into workflows, training, access rights and professional roles."],
    ["Security and Resilience", "SEC", "Protection, availability, recovery, traceability and continuity."],
    ["Feedback, Correction and Learning", "LRN", "Correction pathways, evaluation and institutional learning."],
  ];
  return (
    <section className="section"><div className="container">
      ...
      <Card className="soft-card"><div className="content-block">
        ...
        <div className="button-row"><a className="primary-button" href={TECHNICAL_MANUSCRIPT_URL} download="IML_Technical_Manuscript.pdf">Download the Technical Manuscript (PDF)</a><a className="secondary-button" href={MANUSCRIPT_URL} download="IML_Founding_Manuscript.pdf">Download the Founding Manuscript</a></div>
      </div></Card>
      <div className="tile-grid three-up top-gap-small">{domains.map(([title, symbol, text]) => <Card key={title} className="value-card"><div className="metric-symbol">{symbol}</div><h3>{title}</h3><p>{text}</p></Card>)}</div>
...
function WorldPage() {
  ...
  useEffect(() => {
    ...
    loadGlobalMapProfiles(controller.signal)
      .then((result) => {
        const nextProfiles = asArray(result.profiles)
          .filter((profile) => profile?.iso3)
          .map((profile) => ({
            ...profile,
            iso3: String(profile.iso3).trim().toUpperCase(),
          }));
  ...
  const selectedProfile = selectedCountry
    ? profileByIso3[String(selectedCountry.iso3).trim().toUpperCase()]
    : null;
  ...
  return (
    <section className="section">
      <div className="container">
        <SectionTitle
          badge="Global Map · PostgreSQL test"
          title="Maturity profiles, not country rankings"
          text="Country records are loaded from the PostgreSQL API through ISO alpha-3 codes and connect six domain scores to documented sources, evidence links, limitations and review status."
        />
