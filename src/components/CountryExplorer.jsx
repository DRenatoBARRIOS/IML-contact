  ...
  return (
    <div className="country-explorer">
      <div className="explorer-toolbar">
        ...
        <label><span>Choose a country</span><select value={selectedIso3} onChange={(event) => chooseCountry(event.target.value)}>{countryOptions.map((country) => <option value={country.iso3} key={country.iso3}>{country.name} — {profilesByIso3.has(country.iso3) ? "examined" : "not examined"}</option>)}</select></label>
        {jurisdictionOptions.length ? <label><span>Choose jurisdiction</span><select value={selectedJurisdiction?.id || ""} onChange={(event) => setSelectedJurisdictionId(event.target.value)}>{jurisdictionOptions.map((option) => <option value={option.id} key={option.id}>{option.label} — {option.profileIso3 ? "examined" : "audit in progress"}</option>)}</select></label> : null}
      </div>
      ...
      <div className="explorer-grid">
        <div className="map-panel">
          ...
          <p className="map-caption">Colour distinguishes documentary coverage and groups examined profiles by their provisional orientation signal. It is not a ranking. A neutral country is “not yet examined”, never “low maturity”. The world map remains country-level; subnational profiles are selected from the jurisdiction menu.</p>
        </div>
