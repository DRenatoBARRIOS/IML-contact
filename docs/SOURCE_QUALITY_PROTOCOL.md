...
The Romania manifest is the first fully specified semantic manifest. Other countries remain at earlier review maturity until equivalent manifests are approved.
...
As of 9 August 2026, the Production API contains 16 published country profiles and 81 documentary sources. Romania is the first profile with a source-specific semantic audit manifest. The remaining profiles must be migrated progressively from earlier structural/URL review to the full v2 semantic protocol.
